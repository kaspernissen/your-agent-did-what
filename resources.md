...
- [genainormalizerprocessor (source)](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor/genainormalizerprocessor) — the collector processor that canonicalizes span attributes from **OpenInference** and **OpenLLMetry** into official OTel GenAI semconv (pins schema v1.40.0). Config key is `gen_ai_normalizer`; **traces only**; no auto-detection (you list the source conventions explicitly). Status: merged, **alpha**. Note: it is **not in the `0.153.0` contrib Docker image** — it ships built-in starting `0.154.0`, or you build a custom collector with the OpenTelemetry Collector Builder (`ocb`). Example renames: `llm.model_name`→`gen_ai.request.model`, `llm.token_count.prompt`→`gen_ai.usage.input_tokens`, `openinference.span.kind`/`traceloop.span.kind`→`gen_ai.operation.name`.
