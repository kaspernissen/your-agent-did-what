...
kubectl create configmap production-db-init -n db --from-file=init.sql=postgres/init.sql \
...
kubectl patch deployment production-db -n db --type=merge -p \
  "{\"spec\":{\"template\":{\"metadata\":{\"annotations\":{\"capybara.dev/schema-hash\":\"$(shasum -a 256 postgres/init.sql | cut -c1-12)\"}}}}}" >/dev/null

kubectl rollout status -n db deployment/production-db --timeout=180s
