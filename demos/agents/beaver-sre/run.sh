#!/usr/bin/env bash
# One local run of beaver-sre, outside the cluster.
#
#   ./run.sh "List all the records in the database."
#
# There is no instrumentation switch: this directory *is* the OpenInference agent. For the other
# vocabulary run ../otter-sre/run.sh — same loop, same tools, one library different.
set -euo pipefail
cd "$(dirname "$0")"
...
export AGENT_NAME="${AGENT_NAME:-beaver-sre}"
